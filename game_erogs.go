...
func EnsureGameErogs(gameID int, gameName string, gameImage string, brandID int) (*GameErogs, error) {
	var game GameErogs
	if err := Dbs.Where("id = ?", gameID).
		FirstOrCreate(&game, GameErogs{ID: gameID, Name: gameName, BrandErogsID: brandID, Image: gameImage}).Error; err != nil {
		return nil, err
	}
	return &game, nil
}
...
func EnsureGameErogsTx(tx *gorm.DB, gameID int, gameName string, gameImage string, brandID int) (*GameErogs, error) {
	var game GameErogs
